...
const ProjectCard = ({ index, name, description, tags, image, project_link, link_icon }) => {
  return (
    <motion.div variants={fadeIn("up", "spring", index * 0.5, 0.75)}>
      <Tilt
        ...
        className="bg-[#F0F2F5] p-5 rounded-2xl sm:w-[360px] w-full shadow-lg"
      >
        <div className="relative w-full h-[230px]">
          ...
          <div className="absolute inset-0 flex justify-end m-3 card-img_hover">
            <div
              onClick={() => window.open(project_link, "_blank")} // <-- UBAH INI
              className="black-gradient w-10 h-10 rounded-full flex justify-center items-center cursor-pointer"
            >
              <img
                src={link_icon} // <-- DAN UBAH INI
        ...
        <div className="mt-5">
          <h3 className="text-[#1A1A1A] font-bold text-[24px]">{name}</h3>
          <p className="mt-2 text-[#6B6E70] text-[14px]">{description}</p>
        </div>
